...
enum {
	 ...
	 * this error code. */
	COWSQL_CLIENT_PROTO_RECEIVED_FAILURE = 1,
 ...
 * in milliseconds. */
COWSQL_VISIBLE_TO_TESTS void clientContextMillis(struct client_context *context,
						 long millis);

/* Initialize a new client. */
COWSQL_VISIBLE_TO_TESTS int clientOpen(struct client_proto *c,
				       const char *addr,
				       uint64_t server_id);

/* Release all memory used by the client, and close the client socket. */
COWSQL_VISIBLE_TO_TESTS void clientClose(struct client_proto *c);

/* Initialize the connection by writing the protocol version. This must be
 * called before using any other API. */
COWSQL_VISIBLE_TO_TESTS int clientSendHandshake(struct client_proto *c,
						struct client_context *context);

/* Send a request to get the current leader. */
COWSQL_VISIBLE_TO_TESTS int clientSendLeader(struct client_proto *c,
					     struct client_context *context);

/* Send a request identifying this client to the attached server. */
COWSQL_VISIBLE_TO_TESTS int clientSendClient(struct client_proto *c,
					     uint64_t id,
					     struct client_context *context);

/* Send a request to open a database */
COWSQL_VISIBLE_TO_TESTS int clientSendOpen(struct client_proto *c,
					   const char *name,
					   struct client_context *context);

/* Receive the response to an open request. */
COWSQL_VISIBLE_TO_TESTS int clientRecvDb(struct client_proto *c,
					 struct client_context *context);

/* Send a request to prepare a statement. */
COWSQL_VISIBLE_TO_TESTS int clientSendPrepare(struct client_proto *c,
					      const char *sql,
					      struct client_context *context);

/* Receive the response to a prepare request. */
COWSQL_VISIBLE_TO_TESTS int clientRecvStmt(struct client_proto *c,
					   uint32_t *stmt_id,
					   uint64_t *n_params,
					   uint64_t *offset,
					   struct client_context *context);
...
COWSQL_VISIBLE_TO_TESTS int clientSendExec(struct client_proto *c,
					   uint32_t stmt_id,
					   struct value *params,
					   unsigned n_params,
					   struct client_context *context);
...
COWSQL_VISIBLE_TO_TESTS int clientSendExecSQL(struct client_proto *c,
					      const char *sql,
					      struct value *params,
					      unsigned n_params,
					      struct client_context *context);
...
COWSQL_VISIBLE_TO_TESTS int clientRecvResult(struct client_proto *c,
					     uint64_t *last_insert_id,
					     uint64_t *rows_affected,
					     struct client_context *context);

/* Send a request to perform a query. */
COWSQL_VISIBLE_TO_TESTS int clientSendQuery(struct client_proto *c,
					    uint32_t stmt_id,
					    struct value *params,
					    unsigned n_params,
					    struct client_context *context);
...
COWSQL_VISIBLE_TO_TESTS int clientSendQuerySQL(struct client_proto *c,
					       const char *sql,
					       struct value *params,
					       unsigned n_params,
					       struct client_context *context);
...
COWSQL_VISIBLE_TO_TESTS int clientRecvRows(struct client_proto *c,
					   struct rows *rows,
					   bool *done,
					   struct client_context *context);

/* Release all memory used in the given rows object. */
COWSQL_VISIBLE_TO_TESTS void clientCloseRows(struct rows *rows);

/* Send a request to interrupt a server that's sending rows. */
COWSQL_VISIBLE_TO_TESTS int clientSendInterrupt(struct client_proto *c,
						struct client_context *context);

/* Send a request to finalize a prepared statement. */
COWSQL_VISIBLE_TO_TESTS int clientSendFinalize(struct client_proto *c,
					       uint32_t stmt_id,
					       struct client_context *context);
...
COWSQL_VISIBLE_TO_TESTS int clientSendAssign(struct client_proto *c,
					     uint64_t id,
					     int role,
					     struct client_context *context);

/* Send a request to remove a server from the cluster. */
COWSQL_VISIBLE_TO_TESTS int clientSendRemove(struct client_proto *c,
					     uint64_t id,
					     struct client_context *context);

/* Send a request to dump the contents of the attached database. */
COWSQL_VISIBLE_TO_TESTS int clientSendDump(struct client_proto *c,
					   struct client_context *context);

/* Send a request to list the nodes of the cluster with their addresses and
 * roles. */
COWSQL_VISIBLE_TO_TESTS int clientSendCluster(struct client_proto *c,
					      struct client_context *context);

/* Send a request to transfer leadership to node with id `id`. */
COWSQL_VISIBLE_TO_TESTS int clientSendTransfer(struct client_proto *c,
					       uint64_t id,
					       struct client_context *context);

/* Send a request to retrieve metadata about the attached server. */
COWSQL_VISIBLE_TO_TESTS int clientSendDescribe(struct client_proto *c,
					       struct client_context *context);

/* Send a request to set the weight metadata for the attached server. */
COWSQL_VISIBLE_TO_TESTS int clientSendWeight(struct client_proto *c,
					     uint64_t weight,
					     struct client_context *context);

/* Receive a response with the ID and address of a single node. */
COWSQL_VISIBLE_TO_TESTS int clientRecvServer(struct client_proto *c,
					     uint64_t *id,
					     char **address,
					     struct client_context *context);

/* Receive a "welcome" handshake response. */
COWSQL_VISIBLE_TO_TESTS int clientRecvWelcome(struct client_proto *c,
					      struct client_context *context);

/* Receive an empty response. */
COWSQL_VISIBLE_TO_TESTS int clientRecvEmpty(struct client_proto *c,
					    struct client_context *context);

/* Receive a failure response. */
COWSQL_VISIBLE_TO_TESTS int clientRecvFailure(struct client_proto *c,
					      uint64_t *code,
					      char **msg,
					      struct client_context *context);

/* Receive a list of nodes in the cluster. */
COWSQL_VISIBLE_TO_TESTS int clientRecvServers(struct client_proto *c,
					      struct client_node_info **servers,
					      uint64_t *n_servers,
					      struct client_context *context);

/* Receive a list of files that make up a database. */
COWSQL_VISIBLE_TO_TESTS int clientRecvFiles(struct client_proto *c,
					    struct client_file **files,
					    size_t *n_files,
					    struct client_context *context);

/* Receive metadata for a single server. */
COWSQL_VISIBLE_TO_TESTS int clientRecvMetadata(struct client_proto *c,
					       uint64_t *failure_domain,
					       uint64_t *weight,
					       struct client_context *context);

#endif /* COWSQL_CLIENT_PROTOCOL_H_ */
